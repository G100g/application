...
TeamView.prototype.promise_team_view_details = function(args) {
    ...
    var normalise_departments_func = function(
        my_department,
        ...
        supervised_departments = _.uniq(supervised_departments, function(item) {
            return item.id;
        });
    ...
    var promise_users_and_leaves = promise_departments.map(department =>
...
TeamView.prototype.inject_statistics = function(args) {
    // Validate parameters
    let param_validation = Joi.validate(args, inject_statistics_args_schema, {
        allowUnknown: true
    });
    ...
    let team_view_details = args.team_view_details,
...
TeamView.prototype.restrainStatisticsForUser = function(args) {
    ...
    observer_user.supervised_users.forEach(
        u => (supervised_user_map[u.id] = u)
    );
...
TeamView.prototype.filtrOnlyUserLeaves = function(args) {
    let team_view_details = args.team_view_details,
        observer_user = args.user,
        user = this.user;

    if (this.user.company.hide_other_user_absences) {
        team_view_details.users_and_leaves = team_view_details.users_and_leaves.filter(
            item =>
                item.user.id === observer_user.id ||
                observer_user.supervised_users.some(
